...
### STATE-028 — Pass 10: backend/frontend contract and async-race audit (2026-08-13)

Scope for this pass shifted from frontend state/navigation (Pass 9) to backend↔frontend contract consistency and asynchronous/race behavior, per this pass's brief. Baseline reconfirmed unchanged before any edit (fresh clone at `f48c6fc`): 446 backend tests, 38 frontend tests, clean `tsc -b --noEmit`, `npm run build`, `npx oxlint`. Every route in `ARCHITECTURE.md`'s contract list was checked against `mini_app_api.py`'s dispatch table and `frontend/src/api/client.ts`'s call sites field-by-field. One real race condition and one real (but inert) type mismatch found and fixed:

- **Session state had no protection against out-of-order responses.** `App.tsx` polls `GET /session/current` every 5s via `useSession`'s `refreshSession`, and separately calls `applySession()` directly with the session object already returned by `/call/result`, `/queue/pause`, `/queue/resume`, and `/queue/call-back` (four call sites in `App.tsx`/`Commands.tsx`). Neither path guarded against arrival order: a poll tick fired just before an operator submits a call outcome, slow enough to resolve just *after* `applySession()` already set the fresh post-outcome session, would silently overwrite that fresh state with stale pre-outcome data (old `currentCustomer`, old progress counts, stale `isPaused`) for up to 5 more seconds -- structurally the same "old data silently overwrites new data" bug class as Pass 8's settings finding, just for session state instead of settings state. Not caught by tests (no test exercises overlapping in-flight requests) or by normal usage on a fast connection, only under realistic mobile network jitter -- found by tracing every `refreshSession`/`applySession` call site and asking "what happens if these resolve out of order," not by reproducing it live. Fixed with a monotonic request-id guard in `useSession.ts`: every `refreshSession()` call and every `applySession()` call bumps a ref-held counter, and a `refreshSession()` response is only applied if its counter value is still current -- so a stale, late-arriving poll response can never clobber state that a newer poll or a direct `applySession()` already set.
- **`customerId` in three POST responses was a bare int, not a string.** `/call/start`, `/call/result`, and `/note` all returned `"customerId": customer_id` where `customer_id` is the already-`int()`-cast dispatch-layer value, while `frontend/src/types.ts` declares `customerId?: string` for all three response shapes and every other customer ID on the wire (`Customer.id`, `_customer_payload`'s output) is deliberately stringified. Verified genuinely inert -- grepped the whole frontend for `.customerId` reads and found none, so this never produced a visible bug -- but it's a real, verified frontend/backend contract mismatch of exactly the kind this pass's brief calls out, and the fix is a one-line `str()` per site with zero behavior change for anything that does start reading the field later. Fixed in `mini_app_api.py`'s `start_call`, `submit_call_result`, and `save_note`.
- Verified correct, left unchanged: every other route/field pair in `ARCHITECTURE.md` against `mini_app_api.py` and `client.ts` (session, statistics, settings, search, record, edit, blacklist, phone-blacklist, import, export) -- no other shape or naming mismatches found. `useUpcomingQueue`'s existing `cancelled`-flag pattern already correctly guards its own out-of-order case (re-fetch on customer/count change) -- confirmed as a working prior example of the same problem class, not something needing the same fix twice. Empty-queue, single-customer, and all-blacklisted queue paths in `get_current_session`/`_active_customer`/`queue_upcoming` traced by hand -- `_active_customer` returning `None` for a blacklisted current customer is intentional given `PROJECT_STATUS.md`'s own open "Decisions still needed" entry for blacklisted-customer queue eligibility, not a bug to fix unilaterally.
- Documented, not fixed (low-value / no live impact): `api.getUpcoming()` (the no-`count` single-customer form of `/queue/upcoming`) is defined in `client.ts` but has zero call sites anywhere in the frontend -- `useUpcomingQueue` always passes a count. Left as-is since it mirrors a real, still-supported backend form (`queue_upcoming()` with no `count` arg) and removing exported API surface wasn't this pass's mandate.
...
### STATE-030 — Pass 12: refresh data-loss and outcome-submission race audit (2026-08-14)

Broader-scope integrity audit per this pass's brief (state-consistency, race conditions, data-integrity invariants). Baseline reconfirmed unchanged before any edit (fresh clone at `6b63fcb`): 448 backend tests, 38 frontend tests, clean typecheck/build/lint. Two verified, high-severity bugs found and fixed, both reproduced with standalone scripts before any code change:

- **Refreshing/requeuing a customer on re-import silently destroyed previously known data.** `database.insert_customers`'s refresh path (a loan_number already worked in a prior session, re-imported and requeued -- allowed, expected behavior per its own docstring) overwrote `first_name`, `last_name`, `phone_numbers`, `balance`, `days_overdue`, and every extended/financial field unconditionally, including with blanks, whenever the new import's source simply didn't capture those fields (`validation.normalize_customer` always fills every field, defaulting anything the source omitted to `""`). Reproduced: import a customer with `balance="$5,000.00"`, mark it worked, re-import the same loan_number with only name/phone -> `balance` came back `''`, wiping real financial data an operator relies on. Fixed: the refresh path now coalesces every overwritable field -- a new, non-empty value from the import still takes effect (re-verified: a genuinely updated balance/phone/name on refresh still overwrites correctly), but an empty/omitted field falls back to the existing value instead of blanking it. This does not touch blacklist state, notes, or attempt/event history, all of which were already correctly untouched by this code path (confirmed by inspection: blacklist is a separate column never referenced in the UPDATE; notes/events live in `customer_events`, a separate table `insert_customers` never touches).
- **Rapid double-submission of a call outcome (e.g. a fast double-tap) could double-apply its side effects.** `QueueEngine.apply_action` read the customer's current status, checked it, and only then wrote the new status -- three separate, non-atomic steps. `mini_app_api.py` runs on `ThreadingHTTPServer`, giving every request its own thread, so two near-simultaneous `POST /call/result` requests for the same customer_id genuinely race at the SQLite level: both can read the same pre-transition status before either commits, so both proceed to record an event and (for call_later) increment the attempt count. Reproduced with 5 real concurrent threads calling `apply_action` on the same customer: `attempt_count` came out `5` instead of `1`, with 5 duplicate `customer_call_later` event rows -- directly corrupting the max-attempts enforcement (`get_max_call_attempts`) and inflating statistics. Fixed: `database.update_customer_status` gained an optional `expected_statuses` guard that performs the transition as a single atomic `UPDATE ... WHERE status IN (...)` and reports whether it actually won (SQLite serializes writers, so only the first commit to actually match the WHERE clause succeeds); `apply_action` now only applies its side effects if it won that atomic claim, and a losing concurrent call cleanly falls through to `next_customer()` -- the same no-op path it already used for an unambiguously-already-handled customer. Also applied the same guard to the other `update_customer_status` call site (the invalid-number auto-flag inside `next_customer`) for consistency, at near-zero cost. Re-verified with 5 concurrent threads (uniform and mixed outcomes): exactly `attempt_count=1`, exactly 1 event, regardless of how many requests raced.
- Verified correct, left unchanged: `_active_customer` returning `None` for a blacklisted current customer (already-open, deliberately deferred product decision per PROJECT_STATUS's own "Decisions still needed"); the intra-batch duplicate-loan-number gap (already documented, already deferred, separate root cause from anything touched this pass).
- Verification: `pytest tests/ -q` -- 450 passed (448 baseline + 2 new regression tests: one for the refresh-coalesce fix, one spinning up 5 real threads against the concurrency fix). `cd frontend && npx tsc -b --noEmit && npm run test && npm run build && npx oxlint` -- clean; 38 tests passed (frontend untouched this pass, re-verified anyway).
- Commit: see `git log` for this pass's SHA (pushed and independently verified via GitHub API per `AGENTS.md`).
- Not yet reached this pass (candidates for a follow-up): auth/PII boundary spot-check against the current full route list, file/import security (xlsx/path handling), Telegram runtime error paths, command-system parser edge cases.
...
4. UI Pass 9, Pass 10, Pass 11, and Pass 12 (frontend navigation, backend/frontend contract + async races, import-pipeline duplicate/flagged-row accounting, refresh data-loss + outcome-submission race) — completed; see STATE-027/STATE-028/STATE-029/STATE-030 above.
...
7. Decide the next integrity-audit focus area (candidates: auth/PII boundary spot-check against the current full route list, file/import security, Telegram runtime error paths, command-parser edge cases, AI-parser edge cases specifically, statistics-engine accuracy under real data volume, mobile layout/touch-target audit) or resolve the stale `fix/VERIFY-anim-intensity-doc-drift` branch; run a fresh repository-wide delegation audit after either.

## Delegation audit metadata

- Audit date: 2026-08-14 (updated; original delegation audit was 2026-08-09)
- Audited HEAD before this pass's commit: `6b63fcb` (this pass's own commit SHA — see `git log` for current HEAD)
- Last control-center template update: 2026-08-09
- Backend tests: 450 passing (448 + 2 new regression tests from Pass 12). Frontend tests: 38 passing. Typecheck/build/lint: clean.
- Dependabot PR count and stale-branch triage last confirmed 2026-08-10 (VERIFY-009/010); not re-checked since -- re-verify PR count before acting on it if it becomes relevant again.
...
- Pass 12 (2026-08-14): fixed a refresh-on-reimport data-loss bug and a concurrent outcome-submission race condition; see STATE-030.
